...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["name"]) || empty($_POST["email"]) || empty($_POST["password"])) {
    ...
    if ($statemen->rowCount() > 0) {
      ...
      // Aqui una vez el user se registre crea una session y lo authentica
      ...
      //Inicia la seccion para crear el file con el hash
      session_start();
      // insert the user inside the session. Es decir al registrarnos nos logea tambien.
      $_SESSION["user"] = $user;
